                                    ...
                                    <div class="py-12">
                                        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 flex justify-center items-center">
                                            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg shadow-lg border border-5">
                                                <div class="max-w-7xl p-6 text-gray-900 shadow-md border">
                                                    <form action="{{ route('update', ['id' => $productData->id]) }}" method="POST" class="p-4">
                                                        @csrf
                                                        <div class="mb-4">
                                                            <label for="name" class="block text-gray-700 font-bold mb-2">Name:</label>
                                                            <input type="text" name="name" class="form-control" value="{{ $productData->name }}" placeholder="Name:">
                                                        </div>
                                                        <div class="mb-4">
                                                            <label for="email" class="block text-gray-700 font-bold mb-2">Price:</label>
                                                            <input type="number" name="price" class="form-control" value="{{ $productData->price }}" placeholder="Price:">
                                                        </div>
                                                        <div class="mb-6">
                                                            <label for="message" class="block text-gray-700 font-bold mb-2">Quantity:</label>
                                                            <input type="number" name="quantity" class="form-control" value="{{ $productData->quantity }}" placeholder="Quantity:">
                                                        </div>
